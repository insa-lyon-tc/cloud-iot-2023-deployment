...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        client.subscribe("test/topic")  # Abonnement au sujet "test/topic"
    else:
...
client.connect("localhost", 1883, 60)  # Utilisez localhost si RabbitMQ est sur la même machine

# Boucle principale du client MQTT
client.loop_start()

# Publication d'un message sur le sujet "test/topic"
client.publish("test/topic", image_base64)
