...
kafka_producer = Producer({
        'bootstrap.servers': '10.0.1.178:9092,10.0.1.176:9094,10.0.1.177:9093',
        'client.id': 'python'
    })
topic = ['CIT_CAMERA_PROCESSED_1']
...
#    os.makedirs(save_directory, exist_ok=True)
    for idy, (result_image) in enumerate(results_list):
        print("#######size",sys.getsizeof(base64.b64encode(result_image)))
...
#            result_file_path = os.path.join(save_directory, f"resultiiimage{idy}.jpg")   
            try:
                send_image(kafka_producer, top, compressed3)
            except Exception as e:
                print(f"Erreur lors de l'envoi de l'image sur le topic {top}: {e}")
...
while True:
    try:
        ...
        consumer.subscribe(['CIT_CAMERA_1'])

        results_dir = "topics_consumed"
        os.makedirs(results_dir, exist_ok=True)

        logging.info("Début de la consommation")

        while len(images_bin)<3:
